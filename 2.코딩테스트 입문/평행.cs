...
//※ 공지 - 2023년 2월 14일 테스트 케이스가 수정되었습니다.문제 설명
//점 네 개의 좌표를 담은 이차원 배열  dots가 다음과 같이 매개변수로 주어집니다.
...
public class Solution
{
    public int solution(int[,] dots)
    {
        if ((dots[0, 0] - dots[1, 0]) * (dots[2, 1] - dots[3, 1]) == (dots[0, 1] - dots[1, 1]) * (dots[2, 0] - dots[3, 0]) ||
            (dots[0, 0] - dots[2, 0]) * (dots[1, 1] - dots[3, 1]) == (dots[0, 1] - dots[2, 1]) * (dots[1, 0] - dots[3, 0]) ||
            (dots[0, 0] - dots[3, 0]) * (dots[1, 1] - dots[2, 1]) == (dots[0, 1] - dots[3, 1]) * (dots[1, 0] - dots[2, 0]))
        {
            return 1;
        }
