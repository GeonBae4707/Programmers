...
//예제 3번에서 1, 2, 3등은 각각 1번, 3번, 4번 학생이지만 세 학생 모두 전국 대회에 참석 불가능합니다. 다음으로 4, 5, 6등은 각각 5번, 2번, 0번 학생이고 세 학생 모두 전국 대회에 참석 가능하므로 각각 1순위, 2순위, 3순위로 선발됩니다. 따라서 10000 × 5 + 100 × 2 + 0 = 50200을 return 합니다.문제 설명
//0번부터 n - 1번까지 n명의 학생 중 3명을 선발하는 전국 대회 선발 고사를 보았습니다. 등수가 높은 3명을 선발해야 하지만, 개인 사정으로 전국 대회에 참여하지 못하는 학생들이 있어 참여가 가능한 학생 중 등수가 높은 3명을 선발하기로 했습니다.
...
public class Solution
{
    ...
    public int solution(int[] rank, bool[] attendance)
    {
        ...
        Dictionary<int, Student> dictRank = new Dictionary<int, Student>();

        for (int i = 0; i < rank.Length; i++)
        {
            if (attendance[i])
            {
                dictRank.Add(rank[i], new Student() { index = i, attendance = attendance[i] });
            }
        }

        dictRank = dictRank.OrderBy(x => x.Key).Take(3).ToDictionary(x => x.Key, x => x.Value);

        int r = 0;
        foreach (KeyValuePair<int, Student> item in dictRank)
        {
            if (r == 0)
            {
                answer += 10000 * item.Value.index;
            }
            else if (r == 1)
            {
                answer += 100 * item.Value.index;
            }
            else if (r == 2)
            {
                answer += item.Value.index;
            }
            r++;
        }
